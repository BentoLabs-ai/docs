...
This is an advanced feature. For most apps, an [integration](/python/integrations) or [tracking events](/python/track-ai) is enough. Trajectories matter when one trace must span multiple LLM calls and tool calls within a single logical turn.

When [`bento.instrument()`](/python/integrations) is active, spans the Google ADK integration captures share the trajectory's `trace_id`. One `with bento.begin(...)` block stitches manual and integration spans into one trace.

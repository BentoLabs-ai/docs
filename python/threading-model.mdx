...
The `on_end → emit` code path is entirely synchronous. No `asyncio` anywhere on T_caller. A FastAPI handler that calls `track_ai` 10 times pays 10 × 10us = 100us total on the event loop thread, awaits nothing, and doesn't yield control.

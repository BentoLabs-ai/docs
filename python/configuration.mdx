...
`init()` is optional if you only use `bento.track_ai`. The first call lazy-initializes from env vars. Call `init()` explicitly when you want to pay setup cost up front (cold-start sensitive workloads), pass credentials from code, or **register identity getters** (the next section). For [integrations](/python/integrations), always call `bento.init()` before `bento.instrument()` so the getters are registered before the first captured call.

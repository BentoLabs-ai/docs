...
<AccordionGroup>
  <Accordion title="provider column is empty">
    There is no auto-inference from model name. Pass `provider="anthropic"` (or `openai`, `google`, `aws_bedrock`, etc.) on every `track_ai` call. See [attribute mapping](/concepts/attribute-mapping).
  </Accordion>
